 ...
 * M9958  Get Power Down Data for resume printing
 * M9959  Reset PowerResumeFlag
 *
 * "T" Codes
 *
...
float cartes[XYZ] = { 0 };
  ...
  static bool power_detect = false;
...
#if ENABLED(POWER_DETECT)
  void setup_powerdetectpin(){
    #if ENABLED(ENDSTOPPULLUP_POWER_DETECT)
      SERIAL_ECHOLNPGM("ENDSTOPPULLUP_POWER_DETECT PULLUP");
      SET_INPUT_PULLUP(POWER_DETECT_PIN);
    #else
          SERIAL_ECHOLNPGM("input");
...
/*
*/
inline void gcode_M9959(){
	settings.ResetPowerFlag();
}
    ...
    case 'M': switch (codenum) {
      ...
      case 503: // M503: print settings currently in memory
        ...
        case 605: // M605: Set Dual X Carriage movement mode
          ...
          break;
      #endif // DUAL_X_CARRIAGE
        case 800: // M800: Set Dual X Carriage movement mode
           handle_power_detect();
           break;
      #if ENABLED(LIN_ADVANCE)
...
//3dymy@3dymy.com
  ...
  void handle_power_detect() {
    if (!power_detect) {
	  ...
	  //const uint8_t SDPrint=1;
	  //SERIAL_PROTOCOLPGM("\nSDPrint1:");
	  //SERIAL_PROTOCOL(SDPrint);

      //settings.BeginSDPrint();
	  if(card.sdprinting){
	  //if(true){
	  char* cpFName=card.filename;

	  //SERIAL_PROTOCOL(cpFName);
	  
	  uint32_t fLength=strlen(cpFName);
	  //uint32_t fLength=12;
	  
	  SERIAL_PROTOCOL(fLength);
	  int16_t temp0=0;
	  int16_t tempBed=0;
	  
	  if(EXTRUDERS>0){
		  temp0=thermalManager.degTargetHotend(0);
	  }
	  ...
	  //  bool MarlinSettings::SaveCurrentPos(const char* cpFName,uint8_t iFLen,uint32_t iFPos, float fXPos, float fYPos,float fZPos,float fE0Pos,float fFeedrate,int16_t temp0,int16_t tempBed){
	 ...
	 // SERIAL_PROTOCOL(current_position[E_AXIS]);
	  ...
	  //SERIAL_PROTOCOL(0);
      //settings.ReadXDData();
      //SERIAL_PROTOCOLPGM(",enqueue_and_echo_commands_P:");
      //enqueue_and_echo_commands_P(PSTR(POWER_DETECT_SCRIPT));
      stepper.synchronize();
		}
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
	...
	//when power on,S is LOW，powercount=751；
    ...
    if(powerCount>POWER_DELAY && power_flag==1){
    	SERIAL_ECHOLNPGM("powerCount>POWER_DELAY && power_flag==1");

    	handle_power_detect();
    	power_flag=2;  //release button,detect power and flag=1
    }
//    if(powerCount==0 && power_flag==2){//oh my god,the power is down but i am busy.
//    	power_flag=3;
//    	SERIAL_ECHOLNPGM("powerCount==0 && power_flag==2");
//    	handle_power_detect();
//    }
...
#if ENABLED(EEPROM_SETTINGS)
  if(settings.BeginSDPrint()){
	  SERIAL_PROTOCOLLNPGM("BeginSDPrint True");
  }else{
	  SERIAL_PROTOCOLLNPGM("BeginSDPrint False");
  }

  //if(settings.CheckPowerFlag()){
//	  SERIAL_PROTOCOLLNPGM("\nCheckPowerFlag True");
//  }else{
...
//  }
  ...
  lcd_init();
    ...
    #if ENABLED(DOGLCD)
  SERIAL_PROTOCOLLNPGM("BOOTSCREEN_TIMEOUT:");
  SERIAL_PROTOCOL(BOOTSCREEN_TIMEOUT);
