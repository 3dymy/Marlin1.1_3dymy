...
uint16_t max_display_update_time = 0;
  ...
  void lcd_xd_optoins();
  void lcd_xd_ask();
  void lcd_xd_continue();
  void lcd_xd_continue_screen();
  void lcd_xd_end();
  void lcd_tune_menu();
...
void lcd_status_screen() {
  ...
  lcd_implementation_status_screen();
    ...
    if (lcd_clicked) {
	  ...
	  //3dymy@3dymy.com 2017.09.05
	  if(!pm.isXD()){
		lcd_goto_screen(lcd_main_menu);
	  }else{
		lcd_goto_screen(lcd_xd_ask);
	  }
	  //lcd_synchronize();
    }
...
void lcd_xd_ask(){
	...
	//MENU_BACK(MSG_MAIN);
		...
		u8g.setFont(xd_CN);
	#endif
	//have a printing task!
	u8g.drawStr( 0, 12, MSG_XD_INFO);
	MENU_ITEM_DUMMY();
	
	MENU_ITEM(function, MSG_XD_CONTINUE, lcd_xd_continue);
	MENU_ITEM(function, MSG_XD_CANCLE , lcd_xd_end);
	
	#if ENABLED(XD_CN)
		u8g.setFont(ISO10646_CN);
	#endif
  ...
  void lcd_xd_continue_screen(){
		...
		//Please use software
		//char* cCmd="\x8f\x90\x91\x92\x93\x94\x95";
		
		u8g.drawStr( 15, 20, MSG_XD_USE);
		
		//3DP Tools
		//cCmd="\x96\x97\x98\x99\x9a\x9a\x9b\x9c";
		u8g.drawStr( 35, 40, MSG_XD_3DPTOOLS);
		
		//Operate Continue
		//cCmd="\x9d\x9e\x9f\xa0";
		u8g.drawStr( 35, 60, MSG_XD_OPERATE);
  ...
  void lcd_xd_continue(){
	  //MENU_BACK(MSG_MAIN);
	  lcd_goto_screen(lcd_xd_continue_screen);
